...
if (command === "doctor") {
  ...
  const SCAN_EXTENSIONS = ['.astro', '.html', '.jsx', '.tsx'];

  function findUnprotectedScripts(content: string, filePath: string): Array<{ file: string; src: string }> {
    const results: Array<{ file: string; src: string }> = [];
    const scriptTagRegex = /<script([^>]*)>/gi;
    let m: RegExpExecArray | null;
    while ((m = scriptTagRegex.exec(content)) !== null) {
      const attrs = m[1];
      if (/type=["']text\/plain["']/.test(attrs)) continue;
      const srcMatch = attrs.match(/src=["']([^"']+)["']/);
      if (!srcMatch) continue;
      const src = srcMatch[1];
      if (THIRD_PARTY_DOMAINS.some(d => src.includes(d))) {
        results.push({ file: path.relative(CWD, filePath), src });
      }
    }
    return results;
  }

  const unprotectedScripts: Array<{ file: string; src: string }> = [];
  ...
  function scanDir(dir: string): void {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        unprotectedScripts.push(...findUnprotectedScripts(content, fullPath));
      }
    }
  }
  ...
  checks.push({
    label: 'Unprotected scripts',
    ok: unprotectedScripts.length === 0,
    detail: unprotectedScripts.length === 0
      ? 'No unprotected third-party scripts detected in src/'
      : `${unprotectedScripts.length} third-party script(s) loaded without consent control`
  });

  if (unprotectedScripts.length > 0) {
    const fileList = [...new Set(unprotectedScripts.map(s => s.file))].join(', ');
    findings.push({
      issue: `${unprotectedScripts.length} third-party script(s) load without consent control (${fileList})`,
      fix: 'Add type="text/plain" and data-consent="<category>" to each script tag. See README for GA4 and Meta Pixel examples.'
    });
  }
