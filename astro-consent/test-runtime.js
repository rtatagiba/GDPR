...
(() => {
  ...
  const HEADLINE = "Manage cookie preferences for Test Site";
  const DESCRIPTION = "We use cookies to improve site performance, measure traffic, and support marketing.";
  const ACCEPT_LABEL = "Accept all";
  const REJECT_LABEL = "Reject all";
  const MANAGE_LABEL = "Manage preferences";
  const DEFAULTS = {"essential":true,"analytics":false,"marketing":false};
  const PRESENTATION = "banner";
  const DISPLAY_UNTIL_IDLE = true;
  const DISPLAY_IDLE_DELAY_MS = 1000;
  const STATE_CLASS = "cb-mode-banner";
  ...
  function applyConsent(categories) {
    document.querySelectorAll('script[type="text/plain"][data-consent]').forEach(function(el) {
      var cat = el.dataset.consent;
      if (!categories[cat]) return;
      if (el.dataset.consentApplied === 'true') return;
      el.dataset.consentApplied = 'true';
  ...
  var _existing = read();
  if (_existing) applyConsent(_existing.categories);
})();
