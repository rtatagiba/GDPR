...
> **Language**: Always communicate in **pt-BR** by default unless the user explicitly switches language.
...
- Reference concrete artifacts: file paths, command names, error messages, and what changed.
...
- When context gets large: compress into a brief summary and discard raw noise.
...
- Nunca commitar arquivos `.env`, tokens, API keys ou credentials — sem exceção.
...
- **Nunca commitar direto no `main`**. Sempre criar uma branch e abrir PR.
- Fluxo obrigatório para qualquer mudança:
  1. Criar branch descritiva: `git checkout -b tipo/descricao-curta` (ex: `fix/falsos-positivos`, `feat/batch-generator`)
  2. Commitar na branch com mensagens atômicas e descritivas
  3. Push da branch: `git push -u origin nome-da-branch`
  4. Abrir PR via `gh pr create` com título curto e body descrevendo o que mudou
  5. Retornar a URL da PR para o usuário revisar no GitHub
...
### Plan Template (Cole em `tasks/todo.md`)
...
### Lesson Entry Template (para `tasks/lessons.md`)
- ## [data] — [titulo curto]
- Failure mode:
- Detection signal:
- Prevention rule:
